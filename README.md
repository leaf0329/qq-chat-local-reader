...
仓库目前包含基础解决方案、核心时间范围与同步请求模型、QQ 数据目录与运行版本发现、已通过实际 UAC/VSS 验收的按需提权快照助手、只读密钥候选扫描、不把密钥转换为字符串的 SQLCipher 验证链，以及经过真实账号归属与会话 ID 验收的 QQ 9.9.33 严格适配器。消息正文解析、群名称、索引、MCP 和图形界面仍在实现中。
