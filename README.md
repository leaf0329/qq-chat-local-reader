...
仓库目前包含基础解决方案、核心时间范围与同步请求模型、QQ 数据目录与运行版本发现、已通过实际 UAC/VSS 验收的按需提权快照助手、只读密钥候选扫描、不把密钥转换为字符串的 SQLCipher 验证链、经过真实账号归属与会话 ID 验收的 QQ 9.9.33 严格适配器，以及有界 `40800` 解析器。解析器已支持文本、QQ 表情、引用候选和经验证的媒体最小元数据；规范化消息记录可在同一会话和当前快照内把唯一引用候选解析为稳定消息 ID。首版 SQLCipher 本地索引已支持 DPAPI 当前用户密钥保护、目录访问控制、原子幂等批次和消息往返读取。群名称、搜索、MCP 和图形界面仍在实现中。
