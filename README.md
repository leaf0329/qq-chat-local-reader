...
仓库目前包含基础解决方案、核心时间范围与同步请求模型、QQ 数据目录与运行版本发现、VSS 数据库快照、受限且双向校验身份的按需提权助手、只读密钥候选扫描，以及不把密钥转换为字符串的 SQLCipher 验证链。实际 UAC/VSS 集成验收、账号归属校验、消息解析、索引、MCP 和图形界面仍在实现中。
