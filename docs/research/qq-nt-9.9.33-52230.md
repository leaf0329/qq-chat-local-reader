...
- A seven-natural-day read over both configured acceptance conversation types found nonzero text, QQ-face, reply, image, and video segments. Every non-null body was valid Protobuf wire data. Confirmed media names, sizes, local paths, dimensions, and video durations were present; unsupported extension fields remained partial rather than being treated as complete. Voice duration was deliberately not inferred.
- Group reply candidates resolved by a unique same-group sequence match, and private reply candidates resolved by a unique same-conversation main-table ID match. The acceptance output contained aggregate counts only; no body text, reply summary, nickname, path, account, or conversation identifier was printed or recorded.
